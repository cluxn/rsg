...
- **Tech stack**: Monorepo, single deploy, with separate top-level folders:
  ...
  - **Admin access**: served at path `rsgprofilesheets.com/admin` (not a subdomain). Initial admin account credentials generated by Buildera during setup and handed over to client (not client-supplied upfront).
