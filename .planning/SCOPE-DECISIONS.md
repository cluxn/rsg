...
- **Contact form**: includes a "Product" dropdown (select from the 10 product catalog categories, plus a "General Inquiry" option) so visitors can indicate which product they're asking about. On submission, the form payload is sent to the n8n webhook `https://n8n.srv806265.hstgr.cloud/webhook/rsg-contact-96e0704a-2bd2-4417-82f7-ac753cda3a73` (stored as `CONTACT_FORM_WEBHOOK_URL` in `/backend`'s `.env`), in addition to being recorded as a lead per LEAD-02 (source = "Contact page").
