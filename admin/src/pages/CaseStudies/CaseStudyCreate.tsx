...
export function CaseStudyCreatePage() {
      ...
      <form onSubmit={handleSubmit(d => mutation.mutate(d))}>
        ...
        <div className="flex items-center justify-between px-8 py-3 border-b border-navy/10 bg-white">
          <div className="flex items-center gap-3">
            ...
            <span className="font-heading text-lg text-navy">New Case Study</span>
        ...
        <div className="flex min-h-[calc(100vh-8rem)]">
          ...
          <div className="flex-1 px-8 py-6 overflow-y-auto space-y-5 max-w-3xl">
            <div>
              ...
              <input {...register('title')} onBlur={onTitleBlur} className={inputCls} placeholder="Case study title" />
              {errors.title && <p className="text-red-500 text-xs mt-1">{errors.title.message}</p>}
            </div>

            <div>
              <label className="block text-sm font-semibold text-navy mb-1">Slug *</label>
              <input {...register('slug')} className={`${inputCls} font-mono`} placeholder="case-study-slug" />
            ...
            <div>
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <TiptapEditor value={field.value} onChange={field.onChange} placeholder="Describe the project, challenge and results…" />
                )}
              />
            </div>
          </div>
          ...
          <aside className="w-72 shrink-0 border-l border-navy/10 bg-white">
            <div className="sticky top-0 p-5 space-y-5 max-h-screen overflow-y-auto">
              ...
              {mutation.isError && (
                <p className="text-red-500 text-xs">Failed to create case study. Try again.</p>
              )}
            </div>
          </aside>
        </div>
